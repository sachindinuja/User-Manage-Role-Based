...
function BearerPCR() {
  ...
  const VITE_SIA_BEARER_PCR_URL = import.meta.env.VITE_SIA_BEARER_PCR_URL;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await submitData(VITE_SIA_BEARER_PCR_URL, payload);
  ...
  const getData = async () => {
    ...
    if (statusFilter === "Active") {
      data = await fetchData(VITE_SIA_BEARER_PCR_URL);
    } else {
      data = await fetchInactiveData(VITE_SIA_BEARER_PCR_URL);
    }
    setResponse(data);
  };
  ...
  const data = response
    ...
    .map((item) => ({
      id: item.ID,
      "BEARER PCR ID": item.BEARER_PCR_ID,
      "Tariff ID": item.TARIFF_ID,
      "Tariff Name": item.TARIFF_NAME,
      Rental_Wo_Tax: item.RENTAL_WO_TAX,
      "Service Type": item.SERVICE_TYPE,
      "Order Type": item.ORDER_TYPE,
      "Order Sub Type": item.ORDER_SUB_TYPE,
      speed: item.SPEED,
      "with BB Rate": item.WITH_BB_RATE,
      "without BB Rate": item.WITHOUT_BB_RATE,
      Status: item.STATUS,
    }));
  ...
  const handleUpdate = async (id) => {
    ...
    if (selectedData) {
      setPayload({
        ID: selectedData.ID,
        BEARER_PCR_ID: selectedData.BEARER_PCR_ID,
        ...
        WITH_BB_RATE: selectedData.WITH_BB_RATE,
        WITHOUT_BB_RATE: selectedData.WITHOUT_BB_RATE,
    ...
    if (selectedData && selectedData.STATUS === "Active") {
      ...
      if (result.isConfirmed) {
        try {
          ...
          await updateData(VITE_SIA_BEARER_PCR_URL, id, updatedPayload);
      ...
      >
        <div className="grid w-full grid-cols-3 gap-5 px-10 ml-5">
          <Input
            name="Bearer PCR ID"
            type="text"
            value={payload.BEARER_PCR_ID}
            onChange={(e) =>
              setPayload({ ...payload, BEARER_PCR_ID: e.target.value })
            }
          />
          <Input
            name="Tariff ID"
          ...
          <Input
            ...
            type="text"
            value={payload.SPEED}
            onChange={(e) => setPayload({ ...payload, SPEED: e.target.value })}
          />
          <Input
            name="With BB Rate"
            type="text"
            value={payload.WITH_BB_RATE}
            onChange={(e) =>
              setPayload({ ...payload, WITH_BB_RATE: e.target.value })
            }
          />
          <Input
            name="Without BB Rate"
            type="text"
            value={payload.WITHOUT_BB_RATE}
            onChange={(e) =>
              setPayload({ ...payload, WITHOUT_BB_RATE: e.target.value })
            }
          />
          <Input
            name="Created User"
