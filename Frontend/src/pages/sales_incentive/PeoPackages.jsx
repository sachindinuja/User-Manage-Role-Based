...
function PEOPackagePCR() {
  ...
  const VITE_SIA_PEO_RATES_URL = import.meta.env.VITE_SIA_PEO_RATES_URL;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await submitData(VITE_SIA_PEO_RATES_URL, payload);
  ...
  const getData = async () => {
    ...
    if (statusFilter === "Active") {
      data = await fetchData(VITE_SIA_PEO_RATES_URL);
    } else {
      data = await fetchInactiveData(VITE_SIA_PEO_RATES_URL);
    }
    setResponse(data);
  };
  ...
  const handleUpdate = async (id) => {
    ...
    if (selectedData) {
      setPayload({
        ID: selectedData.ID,
        PEO_PCR_ID: selectedData.PEO_PCR_ID,
    ...
    if (selectedData && selectedData.STATUS === "Active") {
      ...
      if (result.isConfirmed) {
        try {
          ...
          await updateData(VITE_SIA_PEO_RATES_URL, id, updatedPayload);
  ...
  return (
    <div className="block">
      ...
      >
        <div className="grid w-full grid-cols-3 gap-5 px-10">
          <Input
            name="PEO PCR ID"
            type="text"
            value={payload.PEO_PCR_ID}
            onChange={(e) =>
              setPayload({ ...payload, PEO_PCR_ID: e.target.value })
            }
          />
          <Input
            name="Tariff ID"
      ...
      {/* update modal */}
      <ModalPopup
        title="Update PEO Package PCR"
        sideImg={sideImg}
        modalName="updateRates"
      >
        <div className="grid w-full grid-cols-3 gap-5 px-10">
          <Input
            name="PEO PCR ID"
