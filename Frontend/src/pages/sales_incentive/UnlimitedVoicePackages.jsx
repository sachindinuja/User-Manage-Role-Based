...
function UnlimitedVoicePackages() {
  ...
  const VITE_SIA_UNLIMITED_VOICE_URL = import.meta.env
    .VITE_SIA_UNLIMITED_VOICE_URL;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await submitData(VITE_SIA_UNLIMITED_VOICE_URL, payload);
  ...
  const handleAddData = () => {
    ...
    openModal("addUnlimitedVoicePackages");
  };
  ...
  const getData = async () => {
    ...
    if (statusFilter === "Active") {
      data = await fetchData(VITE_SIA_UNLIMITED_VOICE_URL);
    } else {
      data = await fetchInactiveData(VITE_SIA_UNLIMITED_VOICE_URL);
    }
    setResponse(data);
  };
  ...
  const handleUpdate = async (id) => {
    ...
    if (selectedData) {
      setPayload({
        ID: selectedData.ID,
        UVOICE_ID: selectedData.UVOICE_ID,
      ...
      openModal("updateUnlimitedVoicePackages");
    } else {
    ...
    if (selectedData && selectedData.STATUS === "Active") {
      ...
      if (result.isConfirmed) {
        try {
          ...
          await updateData(VITE_SIA_UNLIMITED_VOICE_URL, id, updatedPayload);
  ...
  return (
    <div className="block">
      ...
      {/* Page header */}
      <PageHeader
        title="Unlimited Voice Packages"
      ...
      <ActionTable
        columns={columns}
        data={data}
        title={"Unlimited Voice Package Report"}
      ...
      {/* Add new Data modal */}
      <ModalPopup
        title="Add New Voice Packages"
        sideImg={sideImg}
        modalName="addUnlimitedVoicePackages"
      >
        <div className="grid w-full grid-cols-3 gap-5 px-10">
          <Input
            name="UVOICE ID"
            type="text"
            value={payload.UVOICE_ID}
            onChange={(e) =>
              setPayload({ ...payload, UVOICE_ID: e.target.value })
            }
          />
          <Input
            name="Tariff ID"
          ...
          <Input
            ...
            onChange={(e) => setPayload({ ...payload, MEDIUM: e.target.value })}
          />
          <Input
            name="PCR"
            type="text"
      ...
      {/* update modal  */}
      <ModalPopup
        title="Update Voice Packages"
        sideImg={sideImg}
        modalName="updateUnlimitedVoicePackages"
      >
        <div className="grid w-full grid-cols-3 gap-5 px-10">
          <Input
            name="UVOICE ID"
...
export default UnlimitedVoicePackages;
